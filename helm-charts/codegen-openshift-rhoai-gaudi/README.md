...
Serving runtime template in this example uses model *meta-llama/CodeLlama-7b-hf*.
...
3. Account on https://huggingface.co/, access to model *meta-llama/CodeLlama-7b-hf* and token with Read permissions.
...
```
cd GenAIInfra/helm-charts/codegen-openshift-rhoai-gaudi/
export HFTOKEN="insert-your-huggingface-token-here"
# To run on Gaudi
helm install servingruntime tgi --set global.huggingfacehubApiToken=${HFTOKEN}
```
Verify if template has been created with ```oc get template -A | grep tgi-codellama``` command.
